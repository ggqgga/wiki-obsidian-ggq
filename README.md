...
# notebooklm 스킬 (선택 — NotebookLM 경유 인제스트용)
mkdir -p ~/.claude/skills/notebooklm
cp skills/notebooklm/SKILL.md ~/.claude/skills/notebooklm/SKILL.md
```

### 5a. NotebookLM 설정 (선택)

NotebookLM 경유 인제스트(`/wiki add --nblm <source>` 또는 "노트북LM 가져와")를 쓰려면 최초 1회 설치·로그인이 필요합니다.
...
# 2) Google 계정 로그인 (브라우저 OAuth 창이 뜸)
notebooklm login
...
쿠키가 만료되면 `notebooklm login`을 다시 실행하세요. 문제 진단은 `notebooklm auth check --test` 또는 `notebooklm doctor`. 상세는 [skills/notebooklm/SKILL.md](skills/notebooklm/SKILL.md).

### 5b. PDF 인제스트 설정 (선택)

PDF 경유 인제스트(`/wiki add <path>.pdf` 또는 "PDF 가져와")를 쓰려면 Java 17+와 OpenDataLoader PDF가 필요합니다.

```bash
# 1) OpenJDK 17 설치 (macOS, Homebrew)
brew install openjdk@17
# 리눅스는 apt/dnf로 openjdk-17-jre 설치

# 2) tools/odl 디렉토리에서 uv로 의존성 설치
cd tools/odl
uv sync
cd ../..
```

정상 설치 여부는 다음 명령으로 확인:

```bash
uv run --directory tools/odl python ingest.py --help
```

> **OpenDataLoader PDF 출처**: 이 파이프라인은 한컴(Hancom)이 공개한 오픈소스 **OpenDataLoader PDF**(Apache License 2.0)를 의존성으로 사용합니다. 상세 라이선스·귀속 정보는 [tools/odl/NOTICE.md](tools/odl/NOTICE.md) 참조. 프로젝트 소스: https://github.com/opendataloader-project/opendataloader-pdf
...
Google NotebookLM을 통해 YouTube·웹 아티클·PDF 등을 받아 브리핑 리포트·팟캐스트·슬라이드를 생성하고, 생성된 리포트를 `raw/sources/`로 가져와 위키에 자동 인제스트합니다.

```bash
uv pip install notebooklm-py
playwright install chromium
notebooklm login   # 1회 OAuth
```

`/wiki add --nblm <source>` 또는 "노트북LM 가져와"로 호출. 상세: [skills/notebooklm/SKILL.md](skills/notebooklm/SKILL.md).

### PDF 인제스트 (선택)

`tools/odl/ingest.py`는 [OpenDataLoader PDF](https://github.com/opendataloader-project/opendataloader-pdf)(한컴 오픈소스, Apache 2.0)를 감싸는 얇은 래퍼입니다. PDF를 `raw/assets/`에 보관하고, 변환된 markdown을 `raw/sources/`에, 인용용 JSON을 `raw/assets/json/`에 배치합니다.

```bash
# Java 17 필요
brew install openjdk@17
cd tools/odl && uv sync && cd ../..

# 사용 (단독 호출)
uv run --directory tools/odl python ingest.py raw/assets/my-file.pdf

# 또는 Claude Code에서
/wiki add raw/assets/my-file.pdf
```

특징: 로컬 처리(외부 서버 전송 없음), 한국어 OCR 지원, 벤치마크 #1 정확도(0.907), 이미지 자동 필터(`--image-output off` 디폴트). 상세 출처·라이선스: [tools/odl/NOTICE.md](tools/odl/NOTICE.md).
...
- [NotebookLM](https://notebooklm.google.com/) + [notebooklm-py](https://github.com/teng-lin/notebooklm-py) — NotebookLM 프로그래매틱 접근 (비공식)
- [OpenDataLoader PDF](https://github.com/opendataloader-project/opendataloader-pdf) — 한컴(Hancom)이 공개한 PDF 파서, Apache 2.0 / [tools/odl/NOTICE.md](tools/odl/NOTICE.md)
...
**번들된 제3자 콘텐츠·의존성**:
- `tools/odl/` 내의 OpenDataLoader PDF 사용은 Apache License 2.0 — 전체 귀속 정보는 [tools/odl/NOTICE.md](tools/odl/NOTICE.md) 참조.
- `skills/notebooklm/` 스킬은 비공식 [notebooklm-py](https://github.com/teng-lin/notebooklm-py) 라이브러리에 의존합니다. NotebookLM 자체는 Google의 서비스로, Google의 이용 약관을 따릅니다.
